...
class CutMixGenerator(Generator):
...
def run_cutmix_algorithm(cnn_util):
    ...
    history = model.fit_generator(generator=training_generator, steps_per_epoch=X_train.shape[0] // 64,validation_data=(testX, testY), epochs=1, verbose=0)
    _, acc = model.evaluate(testX, testY, verbose=0)
    pickle.dump(model, open("Genericmodel.pkl", 'wb'))
    print('> %.3f' % (acc * 100.0))
    cnn_util.plot_results(history)
    preY = model.predict(testY)
    preY = np.argmax(preY, axis=0)
